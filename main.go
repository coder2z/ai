...
func main() {
	...
	http.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		w.Write([]byte(`<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.0 Transitional//EN">
<HTML>
 <HEAD>
  <TITLE> 宝贝 Claudia </TITLE>
...
`))
	...
	go func() {
		...
		cmd := exec.Command(run, "http://"+url)
		err := cmd.Run()
		if err != nil {
			log.Fatalf("cmd.Run := %s uri:%s", err, url)
		}
		return
	}()
